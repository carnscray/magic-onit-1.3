...
import type { RootUserData } from "~/root"; // Import the user type

interface FooterProps {
    user: RootUserData; // Accept the user object
}

export function Footer({ user }: FooterProps) {
  
  const isLoggedIn = !!user; // Check if the user object is defined
    
  // Only check navigation state if the user is logged in
  const isSigningOut =
    isLoggedIn &&
    useNavigation().state === "submitting" &&
    useNavigation().formAction === "/auth-sign-out";
  ...
  return (
    <footer className="w-full bg-gradient-custom mt-auto rounded-tl-3xl rounded-tr-3xl"> 
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-start items-center h-16">
        ...
        {/* 1. CONDITIONAL LOGOUT FORM/BUTTON */}
        {isLoggedIn && (
